...
# bash generate-problem.sh subgraph arkavidia-2022-subgraph-divisibility "Subgraph Divisibility" 1 256 final
...
g++ solution.cpp -O2 -Wall -std=c++17 -o solution
...
mkdir "data/secret"
mkdir "data/sample"
SAMPLE_TC_REGEX='[.]*sample[.]*'
for testcases in "../$PROBLEM_SLUG/tc/*"; do
    for testcase in $testcases; do
        if [[ $testcase =~ $SAMPLE_TC_REGEX ]]
        then
            cp $testcase "data/sample"
        else
            cp $testcase "data/secret"
        fi
    done
done
cd "data/secret"
