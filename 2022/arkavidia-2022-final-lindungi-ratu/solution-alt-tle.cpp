...
void SPFA(vector<int>& dist, vector<int>& p) {
    dist = vector<int>(maxn + 1, inf);
    p = vector<int>(maxn + 1, -1);
    dist[source] = 0;
    bool inq[maxn+1] = {};
    queue<int> q;
    q.push(source);
...
int32_t main(){
    ...
    int tn = coordinate0.size();
    int tm = coordinate1.size();
    if(tn > tm){ // if cell that need to fill is more than available point, no solution   
        print(-1)
        return 0;
    }
    ...
    range(i,0,tm)
        range(j,0,tn)
            add_edge(i,j+tm,1,matdist(coordinate1[i], coordinate0[j]));

    range(i,0,tm)
        add_edge(source, i, 1, 0);
    
    range(j,0,tn)
        add_edge(j+tm, sink, 1, 0);


    print(min_cost_flow(tn))
