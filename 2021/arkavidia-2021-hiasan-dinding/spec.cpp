...
protected:
    ...
    void Constraints() {
        ...
        // CONS(isConvexClockwisePolygon(PolygonX, PolygonY));
        // CONS(distinctPointsPolygon(PolygonX,PolygonY));
...
private:
    ...
    bool isConvexClockwisePolygon(vector<long long> x, vector<long long> y){
    ...
    bool distinctPointsPolygon(vector<long long> x, vector<long long> y){
    ...
    bool isValidPolygon(vector<long long> x, vector<long long> y){
        // valid polygon itu punya > 2 vertices, convex, clockwise, mulai dari kiri-bawah, titiknya masuk range, dan semua titiknya distinct
        // cek convex & clockwise 
        int n = x.size();
        if (n < 3) return false;
        for (int i = 0; i < n; i++){
        ...
        // cek titik masuk range
        for (int i = 0; i < n; i++){
            if (x[i] < MINXY || x[i] > MAXXY || y[i] < MINXY || y[i] > MAXXY)
                return false;
        }
        // cek titik distinct
        set<pair<long long, long long>> udah;
...
protected:
    ...
    void TestCases() {
        ...
        CASE(N = 8, Q = 3,
             ...
             QueriesL = {0,3,0},
             QueriesR = {0,3,3});
        CASE(N = MAXN, Q = MAXQ, flatPolygon(PolygonX,PolygonY,N,MINXY,MAXXY,0), randomQueries(QueriesL,QueriesR,Q,MINXY,MAXXY));
        CASE(N = MAXN, Q = MAXQ, flatPolygon(PolygonX,PolygonY,N,MINXY,MAXXY,1), randomQueries(QueriesL,QueriesR,Q,MINXY,MAXXY));
        // middle queries (binary search enforcer)
        for (int i=1; i<=3; i++){
            CASE(N = MAXN, Q = MAXQ, randomConvexPolygon(PolygonX,PolygonY,N,MINXY,MAXXY), enforceBinserQueries(PolygonX,QueriesL,QueriesR,Q,2));
            CASE(N = MAXN, Q = MAXQ, randomConvexPolygon(PolygonX,PolygonY,N,MINXY,MAXXY), enforceBinserQueries(PolygonX,QueriesL,QueriesR,Q,3));
        }
        // random big dense
        for (int i=1; i<=3; i++){
...
private:
    ...
    void randomQueries(vector<long long> &ql, vector<long long> &qr, int q, long long mn, long long mx){
        for (int i=0; i<q; i++){
    ...
    void flatPolygon(vector<long long> &px, vector<long long> &py, int n, long long mn, long long mx, bool vertical){
        if (vertical){
            for (int i=0; i<n; i++) px.push_back(0);
            set<long long> udah;
            for (int i=0; i<n; i++){
                long long y = rnd.nextLongLong(mn, mx);
                while (udah.count(y)) y = rnd.nextLongLong(mn, mx);
                udah.insert(y);
                py.push_back(y);
            }
            ...
            for (int i=0; i<n; i++) py.push_back(0);
            set<long long> udah;
            for (int i=0; i<n; i++){
                long long x = rnd.nextLongLong(mn, mx);
                while (udah.count(x)) x = rnd.nextLongLong(mn, mx);
                udah.insert(x);
                px.push_back(x);
            }
    ...
    void enforceBinserQueries(vector<long long> x, vector<long long> &ql, vector<long long> &qr, int q, int partition){
        sort(x.begin(), x.end());
        int n = x.size();
        int midl, midr;
        if (partition == 2){
        ...
        for (int i=0; i<q; i++){
            int offsetl = rnd.nextInt(0, 10);
            int offsetr = rnd.nextInt(0, 10);
            int l = max(0, midl-offsetl);
            int r = min(n-1, midr+offsetr);
            ql.push_back(x[l]);
            qr.push_back(x[r]);
        }
    }
